...
-- Auto reload if file not modified.
local reload_group = vim.api.nvim_create_augroup("AutoReload", { clear = true })

-- When you gain focus, enter a buffer, or the cursor is idle, check for changes
vim.api.nvim_create_autocmd({ "FocusGained", "BufEnter", "CursorHold", "CursorHoldI", "VimResume" }, {
  group = reload_group,
  pattern = "*",
  -- Check the modification time of the file on disk
  command = "checktime",
})
